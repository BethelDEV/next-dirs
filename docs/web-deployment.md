...
| 公开列表一致性 | 产品公开时，首页、分类、标签和搜索均可找到该产品 |
| 管理隐藏后列表一致性 | 管理员下架/隐藏后，首页、分类、标签和搜索均无法找到该产品 |
| 隐藏后作者编辑 | 作者编辑后显示 `Publish Date: Not published` 和 `Hidden by staff. Edits keep this restriction.`，产品未重新公开 |
| 隐藏后直接访问详情 | 直接访问原产品详情链接返回 404 |
...
本轮列表一致性、隐藏后直接详情返回 404，以及作者编辑保留管理隐藏限制已获开发者反馈。集合/sitemap、管理恢复、支付或旧同步任务相关场景尚未反馈，不由上述结果推断通过。
